...
class CascadeLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, args):
        ...
        for _ in range(args["alpha"]):
            self.convs.append(GCNConv(in_channels, out_channels, cached=False, add_self_loops=False))
...
class LinearCombinationLayer(torch.nn.Module):
    def __init__(self, alpha):
        ...
        self.params = torch.nn.ParameterList([torch.nn.Parameter(torch.randn(1)) for i in range(alpha+1)])
